...
class ImgClassificationNet:
    ...
    def compile(self, save_path="."):
        ...
        try:
            if self.num_pipeline_cores > 1:
                ...
                self.model_neuron = torch.neuron.trace(self.model, self.sample_data, compiler_args=compiler_args)                
            else:
                self.model_neuron = torch.neuron.trace(self.model, self.sample_data)
    ...
    def load(self, filepath):
        ...
        if filepath is not None:
            ...
            self.model_neuron = torch.load(filepath)
        else:
            self.model_neuron = torch.load(self.model_name)
        self.model_neuron_dp = torch.neuron.DataParallel(self.model_neuron)      
        return self.model_neuron
...
class VisionTransformerNet(ImgClassificationNet):
    def __init__(self, model, model_name, model_type="vit", num_pipeline_cores=1, img_size=224):   
        ...
        self.sample_data = self.feature_extractor(torch.randn(3, img_size, img_size), return_tensors="pt")['pixel_values']
    ...
    def get_single_predict_result(self, img):
        ...
        logits = self.predict(x, False).softmax(dim=1)
        labels = preprocess_img.load_imagenet1k_labels()
